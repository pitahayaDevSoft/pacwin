...
function _pw_parse_winget_lines
{
    ...
    foreach ($line in $lines)
    {
        ...
        if (-not $dataStart -or $line -match "^\s*$")
        { continue 
        }
        $len = $line.Length
        if ($len -le $nameOff)
        ...
        try
        {
            ...
            $id = if ($len -gt $idOff)
            { $line.Substring($idOff, [Math]::Min($versionOff - $idOff, $len - $idOff)).Trim() 
            } else
            { "" 
            }
            $ver = if ($len -gt $versionOff)
            { $line.Substring($versionOff, [Math]::Min($vEnd - $versionOff, $len - $versionOff)).Trim() 
            } else
            { "?" 
            }
            if ($name -and $id)
...
function _pw_search_all
{
    ...
    if ($managers["winget"])
    {
        ...
        $jobs["winget"] = Start-Job -ScriptBlock {
            param($exe, $q)
            try
            { & $exe search --query $q --accept-source-agreements 2>$null 
            } catch
            { @() 
            }
    ...
    if ($managers["choco"])
    {
        ...
        $jobs["choco"] = Start-Job -ScriptBlock {
            param($exe, $q)
            try
            { & $exe search $q --limit-output 2>$null 
            } catch
            { @() 
            }
        } -ArgumentList $exe, $query
...
function pacwin
{
    ...
    switch -Regex ($Command)
    {
        ...
        {
            if (-not $Query)
            { _pw_color "  [!] Falta término de búsqueda." Yellow; return 
            }
        ...
        {
            if (-not $Query)
            { _pw_color "  [!] Falta nombre del paquete." Yellow; return 
            }
        ...
        {
            if (-not $Query)
            { _pw_color "  [!] Falta nombre del paquete a desinstalar." Yellow; return 
            }
...
function _pw_do_uninstall
{
    ...
    {
        "winget"
        { $output = winget uninstall --name $name 2>&1 
        }
        "choco"
...
function _pw_do_list
{
    ...
    if ($filter)
    { _pw_color "  (Filtro: '$filter')" DarkGray 
    }
...
function _pw_handle_result
{
    ...
    switch ($manager)
    {
        "winget"
        {
            if ($exitCode -eq 0)
            { $success = $true 
            } elseif ($exitCode -eq -1978335186)
            { $msg = "Requiere reinicio para completar." 
            } # 0x8A15002E
            else
            { $msg = "Error de Winget (Código: $exitCode)." 
            }
        }
        "choco"
        {
            if ($exitCode -eq 0 -or $exitCode -eq 1641 -or $exitCode -eq 3010)
            {
                $success = $true
                if ($exitCode -ne 0)
                { $msg = "Éxito (Requiere reinicio)." 
                }
            } else
            { $msg = "Error de Chocolatey (Código: $exitCode)." 
            }
        }
        "scoop"
        {
    ...
    } else
    {
        ...
        if ($msg)
        { _pw_color "  Detalle: $msg" Yellow 
        } else
        { _pw_color "  Revisa la salida anterior para más detalles." DarkGray 
        }
    }
}
